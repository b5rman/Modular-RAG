...
**Beta version:** v0.2.1-beta.2
**Production base version:** v0.2.0

## Workflows

| File | n8n Workflow ID | Description |
|------|----------------|-------------|
| `RAG Retrieval Sub-Workflow v0.2.1 Beta.json` | `UH7qTvJ71mZAN4Go` | **v0.2.1** — Retrieval agent with Voyage AI reranking + Citation Verification |
| `RAG INGESTION v0.2.1 BETA.json` | `OyJjoBU5OBY2UT9M` | **v0.2.1** — Ingestion pipeline (beta) |
| `RAG Retrieval Sub-Workflow (Beta).json` | `EQTUC67VhmFzuiAB` | v0.2.0 — Retrieval agent with Citation Verification System + extended thinking |
| `RAG INGESTION (Beta - Batch Contextual).json` | `nZ8Gqdhnz32W9le8` | v0.2.0 — Ingestion pipeline with batch contextual embedding route |
| `Contextual Embedding Batch (Beta).json` | `UMq7KbkoShJW9Nt0` | OpenAI Batch API sub-workflow for async contextual embeddings |
...
| Feature | Production (v0.2.0) | Beta |
|---------|---------------------|------|
...
| **v0.2.1 Tests** | | |
| #3514 | Pass | Voyage AI rerank validation: 5 refs, 2 docs, 71s |
| Cross-doc | Pass | Embedding/chunking comparison across 3 documents, 58s |
| Cost numbers | Pass | Exact dollar amounts, percentages, latency benchmarks extracted, 58s |
| Budget reasoning | Pass | $500/month constraint synthesis from 2 documents, 87s |
...
### v0.2.1-beta.2 — 2026-02-26 — TypeVersion Hardening & Expression Fix

**Fixed: Insert into Vector Store expression**
- `"query": "{{ $json.insertQuery }}"` was missing `=` prefix — treated as literal string. Fixed to `"query": "={{ $json.insertQuery }}"`

**Upgraded: 51 node typeVersions across both workflows**
- Ingestion: 39 nodes bumped (If 2.2→2.3, Switch 3.2→3.3, HTTP Request 4.2→4.4, Code 2→2.1, Merge 3→3.1, Set 3.4→3.5, etc.)
- Retrieval: 12 nodes — 11 typeVersion bumps + 1 toolDescription added

**Added: toolDescription to Dynamic Hybrid Search**
- Agent now has clear tool description for hybrid search tool selection

**Upgraded: Major node version bumps in retrieval**
- `Agentic RAG 1` bumped 1.9→3.1
- `Context Expansion` HTTP Request bumped 4.2→4.4

**Validation improvement**
- Ingestion warnings reduced 129→90 (strict profile)
- Retrieval warnings reduced 55→46 (strict profile)

### v0.2.1-beta.1 — 2026-02-26 — Voyage AI Reranker + Bug Fixes

**Changed: Reranker**
- Replaced Cohere rerank-v3.5 with Voyage AI rerank-2.5 (`POST https://api.voyageai.com/v1/rerank`, `top_k: 15`)
- No downstream code changes needed — Voyage response format matches Cohere (`results[].index`, `results[].relevance_score`)
- Removed old "Rerank with Cohere 3.5" node

**Fixed: If node type validation**
- "If" node after "Fetch Metadata Fields1" had strict type validation checking `$json` as `object notEmpty`, which failed when sub-workflow passed empty string. Changed to `string notEmpty` with loose type validation

**Removed: Orphaned nodes**
- Removed disconnected "Create Zep User" node

**New workflow files**
- `RAG Retrieval Sub-Workflow v0.2.1 Beta.json` (n8n ID: `UH7qTvJ71mZAN4Go`)
- `RAG INGESTION v0.2.1 BETA.json` (n8n ID: `OyJjoBU5OBY2UT9M`)

### v0.2.0-beta.1 — 2026-02-25 — Citation Verification System Phase 1
...
### v0.2.0-beta.0 — 2026-02-21 — Batch Contextual Embeddings
